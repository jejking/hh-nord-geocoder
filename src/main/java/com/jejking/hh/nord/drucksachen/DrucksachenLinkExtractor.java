...
/**
 * This is a utility class to extract links web pages with query parameters (not resources,
 * as they unfortunately do not appear to have URIs) which represent <i>Drucksachen</i>
 * in the Hamburg ALLRIS installation.
 * 
 * <p>We assume that we have the ability to append a flag indicating that links to the entire
 * data set can be exported on a single page of HTML (albeit broken HTML) and that such
 * a page may actually be stored locally as a copy.</p>
 * 
 * <p>As each "page" contains its own metadata, we are only looking to extract the relevant set 
 * of links.</p>
 * 
 * @author jejking
 *
